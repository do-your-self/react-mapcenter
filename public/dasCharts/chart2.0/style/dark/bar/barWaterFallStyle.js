...
export default function prepareOption(dataOption) {
    ...
    dataOption.series[0].barMaxWidth = "10";
    dataOption.series[0].stack = "water";


    var newWater = copyDeep(dataOption.series[0]);
    newWater.itemStyle = {
        normal: {
            color: "",
            barBorderColor: "",
    ...
    newWater.tooltip = {};
    newWater.itemStyle.normal.color = "rgba(0,0,0,0)";
    newWater.itemStyle.normal.barBorderColor = "rgba(0,0,0,0)";
    newWater.itemStyle.emphasis.color = "rgba(0,0,0,0)";
    newWater.itemStyle.emphasis.barBorderColor = "rgba(0,0,0,0)";
    newWater.tooltip = {
        show: false
    };
    for (var i = 0; i < newWater.data.length; i++) {
        newWater.data[i].value = newWater.data[i].value*Math.random();
    }

    newObj = copyDeep(dataOption.series[0]);
    newObj.barGap = '100%';
    newObj.xAxisIndex = 1;
    newObj.stack = "";
    newObj.itemStyle = {
    ...
    optionNew.unshift(newWater);


    for (var k = 0; k < optionNew.length; k++) {
        dataOption.series.unshift(optionNew[k]);
...
function themeFactory(exports, echarts) {
    ...
    var dataOjO = {
        ...
        //mobike 颜色
        //  color:["#C54E42","#05A8D1","#D1D1D3","#2EA2AA"],
        categoryAxis: {
    ...
    echarts.registerTheme('barWaterFallStyle', theme);
}
