...
export default  function prepareOption(dataOption) {
    ...
    if (dataOption.series) {
        ...
        //计算数组series中type的类型为bar下的,data下value的最大值，给y轴的max属性使用
        // 同时为Line指定Y轴
        var max = 0;
        var xAxisData = [];
        for (var i = 0; i < dataOption.series.length; i++) {
            ...
            if(dataOption.series[i].type == "line"){
                dataOption.series[i].yAxisIndex = 1;
            }
        }
    ...
    for (var i = 0; i < length; i++) {
        ...
        if (dataOption.series[i].type == "bar") {
            ...
            dataOption.series[i].barGap = "100%";
...
function themeFactory(exports, echarts) {
    ...
    echarts.registerTheme('barLineMultiYaxisStyle', theme);
}
