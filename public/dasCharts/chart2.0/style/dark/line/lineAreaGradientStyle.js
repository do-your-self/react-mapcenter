...
export default  function prepareOption(dataOption) {
  ...
  if (dataOption.series) {
    ...
    for (var i = 0; i < dataOption.series.length; i++) {
      ...
      dataOption.series[i].areaStyle = {
        normal:{
          opacity: "0.4",
            color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
              offset: 0,
              color: dataOption.color[2]
            }, {
              offset: 0.5,
              color: dataOption.color[1]
            },{
              offset: 1,
              color: dataOption.color[0]
            }])
        },
      };
      ...
      dataOption.series[i].areaStyle.opacity = 0.5;
      dataOption.series[i].markPoint = {
        data: [
          {name: legendData[tempLength], value: data[tempLength].value, xAxis: tempLength, yAxis: data[tempLength].value},
        ],
        symbol: 'triangle',
        symbolRotate: 180,
        symbolSize: [10, 10],
...
function themeFactory(exports, echarts) {
  ...
  echarts.registerTheme('lineAreaGradientStyle', theme);
}
